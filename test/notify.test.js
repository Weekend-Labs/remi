...
async function withServer(fn) {
  ...
  const api = startNotifyApi({
    config: { apiPort: 0, apiToken: 'secret-token' }, // port 0 → ephemeral, no clashes
    version: '9.9.9',
...
test('API: health needs no token and reports presence', () => withServer(async (api, port) => {
  const r = await req(port, 'GET', '/health');
  assert.equal(r.status, 200);
  assert.deepEqual(r.json, { ok: true, version: '9.9.9', present: true });
}));
